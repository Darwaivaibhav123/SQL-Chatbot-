...
import pandas as pd
import io
from streamlit_chat import message as st_message  # type: ignore
...
with st.sidebar:
    ...
    # Display database schema in a plain format
    st.subheader("Database Schema")
    st.write("**Tables and Columns:**")
    st.write("1. **Artist:** ArtistId, Name")
    st.write("2. **Album:** AlbumId, Title, ArtistId")
    st.write("3. **Track:** TrackId, Name, AlbumId, MediaTypeId, GenreId, Composer, Milliseconds, Bytes, UnitPrice")
    st.write("4. **Customer:** CustomerId, FirstName, LastName, Country, Email")
    st.write("5. **Invoice:** InvoiceId, CustomerId, InvoiceDate, BillingAddress, Total")
